#!/usr/bin/env python3
"""
nerdymark's GameCube Downloader for ES-DE
Downloads games from Myrient's Redump GameCube NKit RVZ collection
...
# Colors - GameCube Purple theme
THEME = get_theme('gamecube')
ACCENT = THEME['accent']
    ...
    def handle_starttag(self, tag, attrs):
        if tag == 'a':
            for name, value in attrs:
                if name == 'href' and value.endswith('.rvz'):
                    self.in_link = True
...
class GameCubeDownloaderUI:
    def __init__(self):
        ...
        pygame.display.set_caption("nerdymark's GameCube Downloader")
    ...
    def get_existing_games(self):
        ...
        existing = set()
        rom_path = Path(GC_ROM_DIR)
        if rom_path.exists():
            for f in rom_path.iterdir():
                if f.suffix.lower() in ['.rvz', '.iso', '.gcm', '.nkit.iso', '.ciso']:
                    existing.add(f.stem)
        return existing

    def fetch_game_list(self):
        self.draw_message("Connecting to Myrient...", "Fetching GameCube game list, please wait")
        pygame.display.flip()
    ...
    def draw_main_menu(self):
        ...
        draw_title_with_glow(self.screen, self.font_large, "NERDYMARK'S GAMECUBE DOWNLOADER", ACCENT, 20)
        ...
        # Note about RVZ format
        format_note = "Format: NKit RVZ (compressed, works directly in Dolphin)"
        format_surf = self.font_tiny.render(format_note, True, ACCENT_DIM)
        self.screen.blit(format_surf, (self.width//2 - format_surf.get_width()//2, 90))

        search_panel = create_panel(self.width - 100, 40, border_color=ACCENT_DIM if self.keyboard_active else SMOKE_GRAY)
        self.screen.blit(search_panel, (50, 110))
        ...
        self.screen.blit(search_surf, (60, 118))

        list_panel = create_panel(self.width - 80, self.height - 240, border_color=SMOKE_GRAY)
        self.screen.blit(list_panel, (40, 165))
        ...
        if len(self.filtered_games) > self.visible_items:
            ...
            pygame.draw.rect(self.screen, DEEP_GRAY, (self.width - 30, 170, 10, bar_height), border_radius=5)
            pygame.draw.rect(self.screen, ACCENT_DIM, (self.width - 30, 170 + handle_pos, 10, handle_height), border_radius=5)
    ...
    def download_game(self, game_name, href):
        """Download a GameCube game - save the RVZ directly"""
        ...
        dest_path = os.path.join(GC_ROM_DIR, filename)
