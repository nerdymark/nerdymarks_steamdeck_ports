#!/usr/bin/env python3
"""
nerdymark's Atari Jaguar CD Downloader for ES-DE
Downloads games from Myrient's Redump Atari Jaguar CD collection
...
BASE_URL = "https://myrient.erista.me/files/Redump/Atari%20-%20Jaguar%20CD%20Interactive%20Multimedia%20System/"
JAGUARCD_ROM_DIR = "/run/media/deck/SK256/Emulation/roms/atarijaguarcd"
...
# Colors - Gloomy Red theme for Atari Jaguar
...
class JaguarCDDownloaderUI:
    def __init__(self):
        ...
        pygame.display.set_caption("nerdymark's Atari Jaguar CD Downloader")
    ...
    def get_existing_games(self):
        """Get list of already downloaded games (ZIP files in ROM directory)"""
        existing = set()
        rom_path = Path(JAGUARCD_ROM_DIR)
        if rom_path.exists():
    ...
    def get_disk_space(self):
        ...
        try:
            stat = os.statvfs(JAGUARCD_ROM_DIR)
    ...
    def draw_main_menu(self):
        ...
        # Header with glow
        draw_title_with_glow(self.screen, self.font_large, "NERDYMARK'S ATARI JAGUAR CD DOWNLOADER", ACCENT, 20)
    ...
    def download_game(self, game_name, href):
        """Download an Atari Jaguar CD game - just save the ZIP directly"""
        ...
        dest_path = os.path.join(JAGUARCD_ROM_DIR, filename)
    ...
    def run(self):
        os.makedirs(JAGUARCD_ROM_DIR, exist_ok=True)
