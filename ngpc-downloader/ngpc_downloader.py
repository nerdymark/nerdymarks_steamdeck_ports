#!/usr/bin/env python3
"""
nerdymark's Neo Geo Pocket Color Downloader for ES-DE
Downloads games from Myrient's No-Intro collection
"""
...
BASE_URL = "https://myrient.erista.me/files/No-Intro/SNK%20-%20NeoGeo%20Pocket%20Color/"
ROM_DIR = "/run/media/deck/SK256/Emulation/roms/ngpc"
...
class DownloaderUI:
    def __init__(self):
        ...
        pygame.display.set_caption("nerdymark's Neo Geo Pocket Color Downloader")
    ...
    def get_existing_games(self):
        ...
        if rom_path.exists():
            for f in rom_path.iterdir():
                if f.suffix.lower() in ['.zip', ''.ngp', '.ngc', '.ngc']:
                    existing.add(f.stem)
        return existing
    ...
    def fetch_game_list(self):
        ...
        try:
            ...
            self.draw_message("Error", f"Failed to fetch: {str(e)[:50]}")
            pygame.display.flip()
    ...
    def draw_disk_space(self):
        ...
        if total_gb == 0:
            return
        x, y = self.width - 135, 15
        bar_width, bar_height = 120, 16
        fill_pct = (total_gb - free_gb) / total_gb if total_gb > 0 else 0
        fill_color = RED if free_gb < 10 else YELLOW if free_gb < 20 else GREEN
        pygame.draw.rect(self.screen, DARK_GRAY, (x, y, bar_width, bar_height), border_radius=3)
        if fill_pct > 0:
            pygame.draw.rect(self.screen, fill_color, (x, y, int(bar_width * fill_pct), bar_height), border_radius=3)
        pygame.draw.rect(self.screen, GRAY, (x, y, bar_width, bar_height), width=1, border_radius=3)
        label_surf = self.font_tiny.render(f"{free_gb:.0f}GB free", True, WHITE)
    ...
    def draw_main_menu(self):
        ...
        draw_title_with_glow(self.screen, self.font_large, "NERDYMARK'S NEO GEO POCKET COLOR DOWNLOADER", ACCENT, 20)

        stats = f"{len(self.games)} games | {len(self.existing_games)} downloaded | {len(self.filtered_games)} shown"
    ...
    def draw_download_progress(self):
        ...
        bar_width, bar_height = self.width - 200, 40
        bar_x, bar_y = 100, self.height // 2
    ...
    def handle_list_input(self, action):
        if action == "UP" and self.selected_index > 0:
            self.selected_index -= 1
            if self.selected_index < self.scroll_offset:
                self.scroll_offset = self.selected_index
        elif action == "DOWN" and self.selected_index < len(self.filtered_games) - 1:
            self.selected_index += 1
    ...
    def download_game(self, game_name, href):
        ...
        try:
            total_size = 0
            try:
                result = subprocess.run(['curl', '-sI', '-L', download_url], capture_output=True, text=True, timeout=30)
            ...
            proc = subprocess.Popen(['wget', '-O', dest_path, '-q', download_url], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            track_process(proc)
            ...
            while proc.poll() is None:
                ...
                if os.path.exists(dest_path):
                    ...
                    if total_mb > 0:
                        ...
                        self.download_speed = f"{size_mb:.1f} / {total_mb:.1f} MB"
                    else:
                        self.download_speed = f"{size_mb:.1f} MB"
